...
~nael~ is available on [[https://melpa.org/#/nael][Melpa]] (and [[https://stable.melpa.org/#/nael][Melpa-Stable]]) but ~nael-lsp~ not yet.  To install ~nael~ from Melpa, first add as package-archive in your Emacs initialization file.
...
After typing =M-x package-refresh-contents RET= which fetches Melpa's package recipes, choose one of the following approaches:

- Interactively: Type =M-x package-install RET nael RET=.
- With ‘package selection’: Add =(add-to-list 'package-selected-packages 'nael)= to your init file, evaluate it, and type =M-x package-install-selected-packages RET=.
- With ~use-package~: Add =(use-package nael :ensure t)= or similar to your init file and evaluate it.
...
- =C-c C-l= (~lsp~) starts lsp-mode, an alternative LSP client.  The key is only bound if ~nael-lsp~ is autoloaded.
...
To view the complete list of Nael's user option variables from within Emacs, customize the group ~nael~ (and its child-groups) with =M-x customize-group RET nael RET=.  This chapter highlights and explains some user options by examples.
...
** Version 0.7.0 from 2025-12-15

- Remove the ~nael-markdown~ package.  Move its code into ~nael~ package instead.  (Wrapped into ~with-eval-after-load~.)
- ~nael~ is now on [[https://melpa.org/#/nael][Melpa]] and [[https://stable.melpa.org/#/nael][Melpa-Stable]].  ~nael-lsp~ is not there yet.
