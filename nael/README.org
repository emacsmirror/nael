...
First clone the Git repository, then, in your Emacs initialization file, add Nael to ~load-path~ and load it.  For lazy loading the autoloads-file need to be build beforehand.
...
As an exception, Emacs customization groups cannot include keymaps.  You can bind custom keys in ~nael-mode-map~:

#+begin_src emacs-lisp
(keymap-set nael-mode-map "C-c ." #'highlight-symbol-at-point)
#+end_src

To automatically enable features (e.g. abbreviations, Eglot or lsp-mode) when Nael is started, customize ~nael-mode-hook~:
